...
Content that must remain inside both axes.

## Formula / behavior

minimum of width and height ratios.

## Swift usage

`16.fitDp(c)`; may leave unused space.

```swift
let configuration = DimensConfiguration(screenWidth: 390, screenHeight: 844)
let value = DynamicDimens.resolve(16, strategy: .fit, configuration: configuration)
```
